...
function config(client) {
  return {
    ...
    notificationsChannelId: client.appEnv.ticketNotificationsChannelId,
  };
}
...
function memberIsAdmin(member, client) {
  const { adminRoleId } = config(client);
  return (
    member?.roles?.cache?.has(adminRoleId) === true ||
...
function isAdmin(interaction) {
  return (
    memberIsAdmin(interaction.member, interaction.client) ||
    interaction.memberPermissions?.has(PermissionsBitField.Flags.Administrator) === true
  );
}

async function ensureGuild(guildId, client) {
  const { ticketCategoryId } = config(client);
  await prisma.guildConfig.upsert({
    where: { guildId },
    create: { guildId, ticketCategoryId },
...
function buildControlPanel(ticket, guild) {
  const category =
    TICKET_CATEGORIES.find((item) => item.value === ticket.category) ||
    { label: ticket.category, emoji: "🎫" };
  const openedAt = Math.floor(new Date(ticket.openedAt).getTime() / 1000);
  const status = ticket.closed
    ? "🔒 Closed"
    : ticket.escalated
      ? "🚨 Escalated"
      : ticket.locked
  ...
  return new EmbedBuilder()
    ...
    .setDescription(
      "This ticket is private to its opener, the configured admin role, server administrators, and the bot."
    )
    .addFields(
  ...
  return [
    ...
    new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        ...
        .setStyle(ButtonStyle.Secondary),
      new ButtonBuilder()
        .setCustomId(`ticket_removeuser:${ticket.channelId}`)
...
async function refreshControlPanel(channel, ticket) {
  ...
  const panel = messages?.find(
    (item) => item.author.bot && item.embeds[0]?.author?.name === "Ticket Control Panel"
  );
  if (panel) {
    await panel.edit({
      embeds: [buildControlPanel(ticket, channel.guild)],
      components: buildButtons(ticket),
...
async function saveTranscript(channel) {
  const { ticketLogChannelId } = config(channel.client);
...
async function archiveTicketAfterTranscriptFailure(channel, ticket, closedBy) {
  const { adminRoleId } = config(channel.client);
...
module.exports = {
  ...
  async execute(message) {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        "Choose the category that best fits your issue. The created channel is visible only to you, the admin role, server administrators, and the bot."
      );
  ...
  selectMenuHandlers: [
    {
      ...
      async execute(interaction) {
        ...
        const { ticketCategoryId, adminRoleId } = config(interaction.client);
        await interaction.deferReply({ flags: 64 });
        await ensureGuild(interaction.guild.id, interaction.client);
        ...
        await channel.send({
          ...
          embeds: [buildControlPanel(ticket, interaction.guild)],
          components: buildButtons(ticket),
  ...
  buttonHandlers: [
      ...
      async execute(interaction) {
        ...
        if (interaction.user.id !== ticket.userId && !isAdmin(interaction)) {
          return interaction.reply({
            content: "Only the ticket opener, the admin role, or a server administrator can close this ticket.",
            flags: 64,
          });
        }
    ...
    },
    {
      ...
      async execute(interaction) {
        if (!isAdmin(interaction)) {
          return interaction.reply({ content: "Admin role or higher only.", flags: 64 });
        }
        const { adminRoleId } = config(interaction.client);
        ...
        const openerCanManageTickets = memberIsAdmin(opener, interaction.client);
        ...
        await interaction.reply({
          content: locked ? "Ticket locked. Admins can continue replying." : "Ticket unlocked.",
        });
    ...
    },
    {
      ...
      async execute(interaction) {
        ...
        const modal = new ModalBuilder()
          .setCustomId(`ticket_removeuser_modal:${channelId}`)
  ...
  modalHandlers: [
    {
      ...
      async execute(interaction) {
        ...
        const { adminRoleId, notificationsChannelId } = config(interaction.client);
        ...
        if (interaction.channel.id !== channelId) {
          return interaction.reply({
            content: "This escalation does not belong to the current ticket channel.",
            flags: 64,
          });
        }

        const role = await interaction.guild.roles.fetch(adminRoleId).catch(() => null);
        if (!role) {
          return interaction.reply({ content: "The configured admin role no longer exists.", flags: 64 });
        }
        ...
        const notificationEmbed = new EmbedBuilder()
          ...
          .setDescription(`A support ticket has been escalated for <@&${adminRoleId}>.`)
          .addFields(
        ...
        const notifications = await interaction.guild.channels
          .fetch(notificationsChannelId)
          .catch(() => null);
        if (!notifications?.isTextBased()) {
          return interaction.editReply(
            `Ticket escalated for <@&${adminRoleId}> and renamed to **${escalatedName}**, but the notifications channel is unavailable.`
          );
        }
        ...
        try {
          ...
          await interaction.editReply({
            content: `Ticket escalated for <@&${adminRoleId}>, renamed to **${escalatedName}**, and the notification was sent.`,
            allowedMentions: { parse: [] },
          });
        } catch {
          await interaction.editReply({
            content: `Ticket escalated for <@&${adminRoleId}> and renamed to **${escalatedName}**, but the notification could not be sent.`,
            allowedMentions: { parse: [] },
          });
        }
      },
    },
    {
      ...
      async execute(interaction) {
        ...
        if (interaction.channel.id !== channelId) {
          return interaction.reply({
            content: "This user request does not belong to the current ticket channel.",
            flags: 64,
          });
        }
        ...
        if (!/^\d{17,20}$/.test(userId)) {
          return interaction.reply({ content: "Enter a valid Discord user ID.", flags: 64 });
        }
        ...
        const alreadyHasAccess =
          interaction.channel.permissionsFor(member)?.has(PermissionsBitField.Flags.ViewChannel) === true;
        if (alreadyHasAccess) {
          return interaction.reply({ content: "That user already has access to this ticket.", flags: 64 });
        }
        ...
        await interaction.reply({
          content: `<@${member.id}> was added to the ticket.`,
    ...
    },
    {
      ...
      async execute(interaction) {
        ...
        if (userId === ticket.userId) {
          return interaction.reply({
            content: "The ticket opener cannot be removed. Close the ticket instead.",
            flags: 64,
          });
        }
        if (userId === interaction.client.user.id) {
          return interaction.reply({ content: "The bot cannot be removed from its own ticket.", flags: 64 });
        }
        ...
        await interaction.reply({
          content: `<@${userId}> was removed from the ticket.`,
          allowedMentions: { parse: [] },
        });
      },
    },
